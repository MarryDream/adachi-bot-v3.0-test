...
export default class Adachi {
	...
	public run(): BOT {
		...
		Plugin.load( this.bot ).then( ( { renderRoutes, serverRouters, registerCmd } ) => {
			...
			/* 开启前端渲染服务 */
			new RenderServer( this.bot.config.renderPort, this.bot.logger, renderRoutes, serverRouters );
			/* 事件监听 */
	...
	private async execute(
		...
		if ( !checkRes ) {
			...
			if ( groupID === -1 ) {
				...
				await this.bot.client.sendGroupMsg( groupID, [ sdk.segment.at( userID ), sdk.segment.text( " " + msg ) ] );
			}
		}
